...
{
  if (!isRegistered<T>())
    throw std::out_of_range("hlvl::SystemManager: attempted to access an unregistered system");
...
{
  if (!isRegistered<T>())
    throw std::out_of_range("hlvl::SystemManager: attempted to access signature of unregistered system");
...
bool SystemManager::isRegistered() const
{
  static_assert(std::is_base_of<ISystem, T>::value,
    "hlvl::SystemManager: system type must be derived from hlvl::ISystem"
  );
...
{
  if (isRegistered<T>())
    throw std::runtime_error("hlvl::SystemManager: attempted to register previously registered system");
...
{
  if (!isRegistered<T>())
    throw std::out_of_range("hlvl::SystemManager: attempted to remove a non-registered system");
